...
async function api(path,method='GET',body,quietUnauthorized=false){
  ...
  if(!response.ok){if(response.status===401 && !quietUnauthorized && !['/login','/recuperacion','/restablecer'].includes(path)){me=null;navigation();location.hash='login';}throw new Error(data.mensaje||'Operación no autorizada. Recargue la página e inténtelo nuevamente.');}
  return data;
}
const can=p=>me?.permisos.includes(p);
const eventName=type=>({IntentoAcceso:'Intento de acceso',IntervencionAdministrativa:'Intervención administrativa',CierreSesion:'Cierre de sesión',SolicitudRecuperacion:'Solicitud de recuperación'}[type]||type);
function navigation(){
  const current=location.hash.slice(1).split('/')[0]||(me?'inicio':'login');
  const group=(label,links)=>'<h3>'+label+'</h3>'+links.map(([id,text])=>'<a href="#'+id+'" class="'+(current===id||(current==='usuario'&&id==='usuarios')?'active':'')+'">'+text+'</a>').join('');
...
async function render(){
  ...
  navigation();const main=$('#main');main.setAttribute('aria-busy','true');main.innerHTML='<div class="loading">Cargando…</div>';
  $('#breadcrumb').textContent=({'inicio':'Panel principal','login':'Iniciar sesión','usuarios':'Usuarios','registro':'Registrar usuario','sesiones':'Sesiones','contrasena':'Contraseña','auditoria':'Auditoría','politica':'Seguridad','recuperacion':'Recuperación','restablecer':'Restablecer acceso'}[route.split('/')[0]]||'Ficha de usuario');
  const show=html=>{if(version!==routeVersion)return false;main.innerHTML=html;main.setAttribute('aria-busy','false');return true;};
  try{
    if(route==='salir'){await api('/logout','POST',{});me=null;location.hash='login';return;}
    if(route==='login'){
      show('<div class="login-grid"><section class="login-art"><div class="eyebrow">UNIVERSIDAD NACIONAL DEL PACÍFICO</div><div class="monogram">NP</div><div><h2>Un solo acceso.<br>Todos sus servicios.</h2><p>Ingrese de forma segura a su espacio universitario, desde cualquier equipo.</p></div></section><section class="card login-card">'+title('Bienvenido de nuevo','Ingrese sus credenciales institucionales.','UNIVERSIDAD NACIONAL DEL PACÍFICO')+'<form id="login">'+input('identificacion','Identificación institucional','text','','required autocomplete="username" maxlength="40" placeholder="Su identificación"')+input('contrasena','Contraseña','password','','required autocomplete="current-password" maxlength="72" placeholder="Ingrese su contraseña"')+input('equipo','Nombre de este equipo','text','Mi equipo','required maxlength="120"')+'<button type="submit">Iniciar sesión →</button></form><div class="login-links"><a href="#recuperacion">Olvidé mi contraseña</a><a href="#restablecer">Tengo un token</a></div><div class="hint">¿Aún no tiene una cuenta? Solicite su registro al administrador de la institución.</div></section></div>');
    ...
    if(route==='sesiones'){
      const list=await api('/sesiones');
      show(title('Mis sesiones','Cada equipo mantiene una sesión independiente. Puede cerrar la que ya no utilice.')+'<section class="card"><div class="section-top"><h2>Equipos conectados</h2><label class="toggle"><input id="incluir-cerradas" type="checkbox"> Mostrar también las sesiones cerradas</label></div><div id="lista-sesiones"></div></section>');
      const update=()=>{$('#lista-sesiones').innerHTML=sessionsTable($('#incluir-cerradas').checked?list:list.filter(s=>s.estado==='ACTIVA'));actions();};
      $('#incluir-cerradas').addEventListener('change',update);update();return;
    }
    if(route==='usuarios'){
    ...
    if(route==='auditoria'||route.startsWith('auditoria/')){
      const objetivo=route.startsWith('auditoria/')?decodeURIComponent(route.slice(10)):'';
      show(title('Bitácora de auditoría','Consulte los intentos de acceso, las intervenciones, los cierres de sesión y las solicitudes de recuperación.','ÁREA DE SEGURIDAD')+'<section class="card"><form id="filtros"><div class="filter">'+input('usuario','Identificación del usuario','text',objetivo)+'<label>Resultado<select name="resultado"><option value="">Todos los eventos</option><option value="EXITOSO">Acceso exitoso</option><option value="RECHAZADO">Acceso rechazado</option><option value="ENVIADA">Recuperación enviada</option><option value="RECHAZADA">Recuperación rechazada</option><option value="ERROR_ENVIO">Error de envío</option></select></label>'+input('contexto','Equipo o contexto')+input('desde','Desde','datetime-local')+input('hasta','Hasta','datetime-local')+'</div><div class="actions"><button type="submit">Aplicar filtros</button><button type="reset" class="secondary">Limpiar campos</button></div></form></section><section class="card" id="eventos"></section>');
      const query=async values=>{const params=new URLSearchParams();Object.entries(values).filter(([,v])=>v).forEach(([k,v])=>params.set(k,k==='desde'||k==='hasta'?new Date(v).toISOString():v));const events=await api('/auditoria?'+params);if(!$('#eventos'))return;$('#eventos').innerHTML=table(['Fecha y hora','Usuario objetivo','Evento / Resultado','Contexto','Ejecutor / Detalle'],events.map(e=>'<tr><td>'+date(e.fechaHora)+'</td><td>'+esc(e.usuarioObjetivo||'Sistema')+'</td><td>'+esc(eventName(e.tipo))+'<small>'+esc(e.resultado||e.accion||e.motivo)+'</small></td><td>'+esc(e.contexto)+'</td><td>'+esc(e.ejecutor||'')+'<small>'+esc(e.motivoRechazo||e.detalle||(e.tipo==='SolicitudRecuperacion'?e.motivo:'')||'')+'</small></td></tr>'));};
      form('filtros',query);$('#filtros').addEventListener('reset',()=>{requestAnimationFrame(()=>{ $('#filtros').elements.usuario.value='';query({}).catch(e=>notify(e.message,true)); });});await query({usuario:objetivo});return;
    }
...
(async()=>{try{me=await api('/estado','GET',undefined,true);}catch{}await render();})();
